...
Lifecycle
---------
1. ``backstep.session(id).__enter__`` pushes a BackstepSession onto a
...
2. The patched ``create`` method:
      ...
      c. Scans the *response* for ``tool_use`` blocks and registers new
         pending Actions.
3. ``backstep.session(id).__exit__`` flushes any Actions that never
...
def _get_sessions() -> list[BackstepSession]:
    if not hasattr(_local, "sessions"):
        _local.sessions = []
    return _local.sessions


def _active_session() -> BackstepSession | None:
    sessions = _get_sessions()
    return sessions[-1] if sessions else None


# ---------------------------------------------------------------------------
# BackstepSession context manager
# ---------------------------------------------------------------------------

class BackstepSession:
    """Context manager returned by ``backstep.session(session_id)``."""
    ...
    def __init__(self, session_id: str, db: str | None = None) -> None:
        ...
        self._store = None
        if db is not None:
            from backstep.store import BackstepStore  # local import avoids circular dep
            self._store = BackstepStore(db)
    ...
    def _on_tool_call(self, tool_use_id: str, name: str, input_: dict) -> None:
        ...
        from backstep.registry import registry as _registry  # noqa: PLC0415
    ...
    def __enter__(self) -> BackstepSession:
...
def _install_patch() -> None:
    ...
    with _patch_lock:
        ...
        try:
            ...
            raise RuntimeError(
                "backstep requires the 'anthropic' package: pip install anthropic"
            ) from exc
...
def _patched_create(self, *args: Any, **kwargs: Any) -> Any:
    ...
    if session is None:
        # No active backstep session — passthrough
